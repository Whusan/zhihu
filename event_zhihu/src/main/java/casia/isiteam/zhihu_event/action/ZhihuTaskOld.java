...
import casia.isiteam.zhihu_event.util.FileIOUtil;
import casia.isiteam.zhihu_event.util.MD5Util;

public class ZhihuTaskOld implements Runnable {
	...
	private static Logger logger = Logger.getLogger(ZhihuTaskOld.class);
	public Date startTime = null;

	public ZhihuTaskOld(EventKeywords eventKeywords) {
	...
	public void run() {
		...
		try {
			List<String> readFileByLine = FileIOUtil.readFileByLine("conf/zhihu.txt");
			boolean contains = readFileByLine.contains(eventKeywords.getKid() + "");
			if (!contains) {
				logger.info("知乎" + "\t历史数据\t采集开始\t关键词:" + eventKeywords.getKeywords());
				int i = 1;
				while (true) {
					String url = "https://www.zhihu.com/api/v4/search_v3?t=general&q="
							+ convertUrl(eventKeywords.getKeywords(), "UTF-8") + "&correction=1&offset=" + (i - 1) * 20
							+ "&limit=20&show_all_topics=0";
					...
					logger.info("知乎" + "\t历史数据\t第" + i + "页\t关键词: " + eventKeywords.getKeywords() + "\t共有 "
							+ urlList.size() + "\t实采 " + success + "\t存在 " + existCount);
				...
				logger.info("知乎" + "\t历史数据\t结束采集\t关键词: " + eventKeywords.getKeywords());
				FileIOUtil.writeFileAdd("conf/zhihu.txt", eventKeywords.getKid() + "");
			}else {
				logger.info("知乎" + "\t历史数据\t已采集\t关键词:" + eventKeywords.getKeywords());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
